...
JZZ_.MidiFile.prototype = (function (p) {
    function f() {};
    f.prototype = p;
    return new f();
})(Array.prototype);
...
JZZ_.MidiFile.prototype.toString = function () {
...
JZZ_.MidiFile.prototype.player = function () {
    ...
    for (i = 0; i < this.length; i++)
        if (this[i] instanceof JZZ_.MidiFile.MTrk) tt.push(this[i]);
    pl.tracks = tt.length;
    ...
    while (true) {
        ...
        for (i = 0; i < tt.length; i++) {
            while (pp[i] < tt[i].length && tt[i][pp[i]].time == t) {
                ...
                for (var attr in obj)
                    if (obj.hasOwnProperty(attr)) evt[attr] = obj[attr];
                evt.track = i;
                pl.push(evt);
                pp[i]++;
            }
...
JZZ_.MidiFile.Chunk.prototype = (function (p) {
    function f() {};
    f.prototype = p;
    return new f();
})(Array.prototype);
JZZ_.MidiFile.Chunk.prototype.sub = {
    'MThd': function () {
        throw new Error("Illegal chunk type: MThd");
    },
    'MTrk': function (t, d) {
        return new JZZ_.MidiFile.MTrk(d);
    }
};
...
JZZ_.MidiFile.MTrk.prototype = (function (p) {
    function f() {};
    f.prototype = p;
    return new f();
})(JZZ_.MidiFile.Chunk.prototype);
...
JZZ_.MidiFile.MTrk.prototype.getTime = function () {
...
JZZ_.MidiFile.MTrk.prototype.addMidi = function () {
    ...
    var msg = JZZ_.MIDI.apply(undefined, args)
        .data();
...
JZZ_.MidiFile.MTrk.prototype.addName = function (t, str) {
    this.addEvent(t, '\xff\x03', str);
}
JZZ_.MidiFile.MTrk.prototype.addText = function (t, str) {
...
JZZ_.MidiFile.Event.prototype.toString = function () {
    ...
    if (s0 > 0x7f && s0 != 0xff && s0 != 0xf0 && s0 != 0xf7) return JZZ_.MIDI(this.status + this.data)
        .toString();
    if (s0 == 0xff) {
        ...
        if (s1 == 0) {
            ...
            str += 'SMPTE offset: ' + (this.data.charCodeAt(0) > 9 ? '' : '0') + this.data.charCodeAt(0)
                .toString();
    ...
    for (var i = 0; i < this.data.length; i++) str += ' ' + (this.data.charCodeAt(i) > 15 ? '' : '0') + this.data.charCodeAt(i)
        .toString(16);
    return str;
};
...
JZZ_.MidiFile.Player.prototype = (function (p) {
    function f() {};
    f.prototype = p;
    return new f();
})(Array.prototype);
JZZ_.MidiFile.Player.prototype.loop = function (b) {
...
JZZ_.MidiFile.Player.prototype.tick = function () {
    ...
    for (; this.ptr < this.length; this.ptr++) {
        ...
        if (e.status == '\xff\x51' && this.ppqn) {
            ...
            evt.midi = JZZ_.MIDI(e.data);
        } else if (e.status.charCodeAt(0) != 0xff) {
            evt.midi = JZZ_.MIDI(e.status + e.data);
        } else {
            evt.status = e.status;
            evt.data = e.data;
        };
